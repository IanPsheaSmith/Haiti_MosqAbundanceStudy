...
```{r Loading in Data}
  ...
  # Define GitHub base URL for raw files
    base_url <- "https://raw.githubusercontent.com/IanPsheaSmith/Haiti_MosqAbundanceStudy/main/Data_and_Code/"
  ...
  # Download Shapefiles for the map(s)
    zip_url <- "https://github.com/IanPsheaSmith/Haiti_MosqAbundanceStudy/raw/main/Data_and_Code/Haiti_Shapefiles.zip"
  ...
  # Base URL for raw GitHub files
    base_url <- "https://raw.githubusercontent.com/IanPsheaSmith/Haiti_MosqAbundanceStudy/main/Data_and_Code/Rasters/"
...
all_lc_variables <- c("LC_shrubs", "LC_cropland", "LC_wetland", "LC_built")
...
cat("\nBootstrap BRT System:", n_bootstrap, "replicates per species\n\n")
...
fit_bootstrap_ensemble <- function(species, dataset_name, 
  ...
  bootstrap_results <- lapply(1:n_boot, function(i) {
    if (i %% 25 == 0) cat("  Progress:", i, "/", n_boot, "\n")
  ...
  cat("  Complete:", n_successful, "/", n_boot, "models in", round(elapsed, 1), "seconds\n")
  
  if (n_successful < n_boot * 0.5) {
    cat("  WARNING: Low success rate\n")
  }
  ...
  if (length(oob_metrics_list) > 10) {
    ...
    cat("  OOB Metrics:\n")
    invisible(apply(aggregated_metrics, 1, function(row) {
      cat(sprintf("    %s: %.3f [%.3f-%.3f]\n",
                  row["metric"], as.numeric(row["mean"]),
...
# =============================================================================
# =============================================================================
# BOOTSTRAP BRT ENSEMBLE PREDICTIONS
# =============================================================================
# =============================================================================
...
prediction_base <- "https://raw.githubusercontent.com/IanPsheaSmith/Haiti_MosqAbundanceStudy/main/Predictions/"
...
output_dir <- "https://raw.githubusercontent.com/IanPsheaSmith/Haiti_MosqAbundanceStudy/main/Predictions/PDPlots"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
